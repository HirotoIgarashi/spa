  ...
  connect : function ( server ) {
    ...
    io
      ...
      .on( 'connection', function ( socket ) {
        ...
        socket.on( 'update:localBusiness', function ( local_business_map ) {
          var local_business_id;

          local_business_id = local_business_map._id;

          delete local_business_map._id;

          crud.update(
            'localBusiness',
            { _id : makeMongoId( local_business_id ) },
            local_business_map,
            function ( result_map ) {
              local_business_map._id = local_business_id;
              result_map.update_object = local_business_map;
              socket.emit( 'localBusiness:update', result_map );
            }
          );
        });

        socket.on( 'destroy:localBusiness', function ( local_business_list ) {
          var local_business_map = local_business_list[ 0 ];

          crud.destroy(
            'localBusiness',
            { _id : makeMongoId( local_business_map._id ) },
            function ( result_map ) {
              var return_map;

              // 削除したオブジェクトを追加する。
              result_map.delete_object = local_business_map;

              socket.emit( 'localBusiness:destroy', result_map );
            }
          );
